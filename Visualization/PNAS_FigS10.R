...
for (chr_base in chrms) {
  ...
  for (i in seq_along(hap_chrs)) {
    ...
    p <- ggplot(dt_chr, aes(x = (start + end) / 2 * 100, y = ratio)) +
      ...
      geom_line(aes(group = Sample), color = palette[i], linewidth = linesize * 72 / 96, alpha = 0.9) +
...
ggsave("All_chr_copyratio_HUANG.pdf", p_all, width = 20, height = 22, units= "cm", paper = 'a4')

shell.exec("All_chr_copyratio_HUANG.pdf")
